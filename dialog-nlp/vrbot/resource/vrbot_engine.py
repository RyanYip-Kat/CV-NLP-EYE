...
def save_model(model, epoch, step, ckpt_filename):
        dump_dict = {
            "epoch": epoch,
            "step": step,
...
class VRBotEngine(BaseEngine):
    ...
    def train(self, dataset=None, start_epoch=0, end_epoch=50, state_pretrain_epoch=20):
        ...
        for e in range(epoch_point, end_epoch):
            ...
            if (e+1)%10==0 or (e+1)==end_epoch:
                ckpt_filename=DO.ckpt_filename_template.format(model="each-vrbot",uuid=TO.task_uuid,epoch=e+1,global_step=self.global_step,metric="train")
                mkdir(os.path.dirname(ckpt_filename))
                save_model(self.model,e+1,self.global_step,ckpt_filename)
    ...
    def hidden_state_mask(pv_hidden, pv_state, inherited, materialistic):
        #print(inherited.unsqueeze(0).unsqueeze(2).shape)
        ...
        #if isinstance
        #if len(reserved_batch_index)==0:
        #    reserved_batch_index=torch.arange(0, B, dtype=torch.long, device=TO.device)
        #elif torch.max(reserved_batch_index) >= B or torch.max(reserved_batch_index) >= pv_hidden.shape[1]:
        #    reserved_batch_index=reserved_batch_index[:-1]
        #else:
        #    reserved_batch_index=reserved_batch_index

        reserved_hidden = pv_hidden[:, reserved_batch_index, :]
        reserved_state = pv_state[reserved_batch_index, :]
        #print(reserved_hidden.shape)
        #print(reserved_state.shape)
