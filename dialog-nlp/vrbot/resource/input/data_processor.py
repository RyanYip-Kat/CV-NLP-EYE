...
class DataProcessor:
    ...
    def get_session(self):
        def _extract_pure_sessions(sessions):
            if self.task.startswith("meddg") or self.task.startswith("eyeai"):
                pure_sessions = []
    ...
    def get_vocab(self):
        ...
        reserved_words += intention2str
        #vocab_df = pd.read_csv(self.vocab_csv_filename.format(dataset=self.task),encoding='utf-8',quoting=csv.QUOTE_NONE)
        try:
            vocab_df = pd.read_csv(self.vocab_csv_filename.format(dataset=self.task))
        except :
            vocab_df = pd.read_csv(self.vocab_csv_filename.format(dataset=self.task),quoting=3)
