...
from resource.input.cutsom_processor import QAKGG,QAIB
#from resource.util.querygeneration import Q2Qpredict
from resource.util.extract_keyword_from_sentence import get_abstract,get_keywords 

q2q_model_ckpt="/path/Query2QueryModel/model.ckpt-850000"  #  QueryGeneration pretrained model path
q2q_model_fn="/home/jovyan/work/QueryGeneration/configs/mega.json"  # QueryGeneration  model  config

#intention2str = ["[CHITCHAT]", "[ASK_SYMPTOMS]", "[DIAGNOSIS]", "[PRESCRIBE]","[RISKA]","[RISKB]"]
...
def cut_seq(sequence):
    seq_list=jieba.cut(sequence,cut_all=False)
...
class SessionProcessor:
    ...
    def process(self, session, supervised=False):
        ...
        tmp_response_type = None
        for i in range(0, len(session), 2):
            if len(session) < 4 or len(session)%2!=0:
                continue
            pv_r_u = [x[0] for x in session[max(0, i - 1):i + 1]]  # r_{t-1}, u_{t}
            pv_r_u_s = "".join(pv_r_u[0]) # # r_{t-1},u_{t}  sentence
            
            # change history rule
            #if (tmp_response_type!="chitchat") and  (tmp_response_type is not None):
            #   q2q = Q2Qpredict(text=pv_r_u_s[0],model_config_fn=q2q_model_fn,model_ckpt=q2q_model_ckpt,minlen=7)[1]
            #   q2qa = QAIB(q2q)  #  use q2q find the answer
            #   pv_r_u =[cut_seq(q2q)] + [cut_seq(q2qa)]
            ...
            intention_spa = [0 if intention_idx != i else 1 for i in range(TO.n_intention)]  # one-hot
            ...
            if (len(session[i]) >= 3) and supervised:
                s_key=None
                if TO.add_state:  # whether add state
                    state_text = "".join(session[i][0])
                    s_key = get_keywords(state_text)
                     
                state_label = [self.inner_vocab.word2index(x) for x in session[i][2]]
                if len(state_label) < DO.state_num:
                    if s_key is not None:
                        s_x = setdiff(s_key,state_label)
                        if len(s_x) > 0:
                            s_n = min(len(s_x),DO.state_num-len(state_label))
                            s_key = np.random.choice(s_x,s_n,False).tolist() # random choice 
                            #if s_key in self.inner_vocab.word2idx:
                            state_label = state_label + [self.inner_vocab.word2index(x) for x in s_key if x in self.inner_vocab.word2idx]

                state_label = state_label + [self.inner_vocab.pad_id] * (DO.state_num - len(state_label))

                a_key=None
                if TO.add_action:  # whether add action
                    action_text = "".join(session[i + 1][2])
                    a_key = get_keywords(action_text)
                action_label = [self.inner_vocab.word2index(x) for x in session[i + 1][2]] 
                if len(action_label) < DO.action_num:
                    if a_key is not None:
                        a_x = setdiff(a_key,action_label)
                        if len(a_x) > 0:
                            a_n = min(len(a_key),DO.action_num-len(action_label))
                            a_key = np.random.choice(a_x,a_n,False).tolist()
                            #if a_key in self.inner_vocab.word2idx:
                            action_label = action_label + [self.inner_vocab.word2index(x) for x in a_key if x in self.inner_vocab.word2idx]
