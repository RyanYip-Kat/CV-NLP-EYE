...
class VRBotBeamSearchDecoder:
    ...
    def forward(self, hidden, inputs, hs_vectors,
        ...
        for i in range(self.max_decode_step):
            if i == 0:
                ...
                next_hidden = nested_index_select(reshaped_hidden,torch.floor_divide(topk_indices,self.beam_width).long())
    ...
    def harvest(self, scores, history, word_index, obs):
        ...
        eos_x =torch.floor_divide(eos_indices,self.beam_width).long()
        #batch_index = ((eos_indices / (self.beam_width * self.beam_width)).long()).cpu().numpy().tolist()
        batch_index = torch.floor_divide(eos_indices,self.beam_width * self.beam_width).long().cpu().numpy().tolist()
