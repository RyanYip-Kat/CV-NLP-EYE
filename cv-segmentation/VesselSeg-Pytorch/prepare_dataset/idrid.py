...
def get_data(data_path,split="train",disease="EX",outdir="data_path_list/IDRID"):
    disease_dict={"HE":"Haemorrhages","EX":"HardExudates","MA":"Microaneurysms","OD":"OpticDisc","SE":"SoftExudates","EXHE":"EXHE"}
    anno_path = join(data_path,"annotations",split,disease_dict[disease])
    img_path = join(data_path,"images",split)
    mask_path = join(data_path,"masks",split)
    
    anno_files = os.listdir(anno_path)
    anno_names = [ x.split("_")[0]+"_"+x.split("_")[1] for x in anno_files]
...
def preprocess_idrid_annotations(data_path,split="train",disease="EX"):
...
if __name__=="__main__":
    data_path = "/path/vesselSeg-Idrid/"
    split = "val"
    disease = "EXHE" 
    get_data(data_path,split,disease,"data_path_list/IDRID_EXHE")
