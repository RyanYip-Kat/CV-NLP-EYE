...
class Evaluate():
    ...
    def add_batch(self,batch_tar,batch_out):
        ...
        #self.target[np.isnan(self.target)] = 0.0
        #self.output[np.isnan(self.output)] = 0.5
        #self.target[np.isinf(self.target)] = 0.5
        #self.output[np.isinf(self.output)] = 0.5
    ...
    def confusion_matrix(self):
        ...
        #self.target = [1.0 if x > 0.0 else 0.0 for x in self.target ] # fixed bug Classification metrics can't handle a mix of continuous and binary targets
