...
def load_data(data_path_list_file,img_size=None):
    ...
    for i in range(len(img_list)):
        ...
        gt = np.asarray(readImg(gt_list[i],img_size))
        if len(gt.shape)==3:
            gt = gt[:,:,0]
        fov = np.asarray(readImg(fov_list[i],img_size))
        if len(fov.shape)==3:
            fov = fov[:,:,0]
...
def get_data_test_overlap(test_data_path_list,img_size, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original, test_masks, test_FOVs= load_data(test_data_path_list,img_size)
