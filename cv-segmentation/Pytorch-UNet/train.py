...
def train_net(net,
    ...
    dataset = BasicDataset(dir_img, dir_mask, img_scale,mask_suffix,img_size,n_channels,n_classes)
    ...
    if dir_img_tst is None and dir_mask_tst is None:
        ...
        #except (AssertionError, RuntimeError):
        val_set = BasicDataset(dir_img_tst, dir_mask_tst, img_scale,mask_suffix,img_size,n_channels,n_classes)
        train_set = dataset
    ...
    class_weights=torch.FloatTensor([0.9, 0.1]).cuda()
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    ...
                    loss = criterion(masks_pred, true_masks) \
                           + dice_loss(F.softmax(masks_pred, dim=1).float(),
                                       ...
                                       multiclass=True) #+ \
                                               #focalloss(masks_pred,true_masks)
                    #loss = focalloss(masks_pred,true_masks) + dice_loss(F.softmax(masks_pred, dim=1).float(),F.one_hot(true_masks, net.n_classes).permute(0, 3, 1, 2).float(),multiclass=True)
